...
const unsigned char crc_table[] = { 0, 94, 188, 226, 97, 63, 221, 131, 194, 156,
...
void timer_test(int udelay) {
	HAL_GPIO_WritePin(gpio, pin, GPIO_PIN_RESET);
	delay_us(timer, udelay);
...
bool one_wire_reset_pulse() {
	...
	delay_us(timer, 70);
	int bit = HAL_GPIO_ReadPin(gpio, pin);
...
bool one_wire_read_bit() {
	...
	static int bit;
	bit = HAL_GPIO_ReadPin(gpio, pin);
	//bit = GPIO_ReadInputDataBit(gpio, pin);
	HAL_GPIO_WritePin(GPIOC, GPIO_PIN_9, bit);
